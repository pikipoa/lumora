...
## 自律開発プロトコル（Autonomous Development Protocol）

Claude Codeはこのリポジトリの実装担当エージェントである。目的は、与えられたTaskを
「コードを書いて終わり」にせず、**実装・実行・検証・修正のループを自律的に回し、
Acceptance Criteriaを満たすこと**である。

### 基本ループ

すべてのTaskについて、以下の順序で進める。
...
5. 仮説を検証する最小実験を行う
6. 必要最小限のコード変更を行う
7. Build / Test / Lint / Runtime verification を実行する
8. 失敗した場合は、失敗原因を分析して修正する
9. 再度テストする
10. Acceptance Criteriaを1項目ずつ検証する
11. Evidenceを残す
12. すべてPASSした場合のみTaskを完了とする

### 重要ルール

**1. 「直ったと思う」をPASSにしない**

コードを変更しただけでは修正完了と判断してはならない。必ず**実行可能なEvidence**を取得する。
（test result / build result / lint result / runtime log / screenshot / reproduction result /
before-after comparison など）

**Evidenceが存在しない場合はPASSではなく`UNVERIFIED`とする。**

**2. 推測と事実を分離する**

以下のラベルを使って明確に分ける。ラベルは日本語に訳さずこのまま使う。

| ラベル | 意味 |
|---|---|
| `OBSERVED` | 実際に観測した事実 |
| `HYPOTHESIS` | 原因として推測していること |
| `EXPERIMENT` | 仮説を検証するために行う実験 |
| `EVIDENCE` | 実験によって得られた証拠 |
| `CONCLUSION` | 証拠から合理的に判断できる結論 |

**3. 失敗したら同じ修正を繰り返さない**

Testが失敗した場合、次の変更に移る前に整理する。
- 何が期待値と違ったのか
- なぜ失敗したのか
- 現在の仮説は何か
- 次に何を観測すべきか
...
原因が特定できていない状態で大規模なリファクタリングを行わない。まず原因を特定するための
最小実験を優先する。**計測装置が調査対象より大きくなってはならない**——切り分け用のフラグ・
HUD・トレースは、安い確認から高い確認へ進む順序を崩してまで積み増さない。

**5. 既存機能を壊さない**

変更後は、可能な範囲で関連するRegression Testを実行する。新しいTestが必要なら追加する。

**6. 人間に質問する前に調査する**

情報が不足していても、まず自律的に確認する：repository / git history / existing tests /
logs / configuration / related implementation / existing documentation。

**自分で確認できる情報について質問してはならない。**
ただし、**仕様上の意思決定**が必要な場合、または**破壊的変更**が必要な場合は人間に確認する。

### Task完了条件

以下を**すべて**満たした場合のみ`DONE`とする。1つでも欠けていれば`DONE`にしない。

- Root causeまたは十分に検証された原因仮説がある
...
ROOT CAUSE:    特定できた原因
CHANGES:       変更した内容
VERIFICATION:  実行したテスト・検証
EVIDENCE:      検証結果
REMAINING RISK: 残っている不確実性
NEXT:          次に必要な作業
```

### 既存ルールとの接続

このプロトコルは**進め方**を定めるものであり、**何を作ってよいか**の判断は下位の各節が優先する。
競合した場合の解釈は次の通り。

| 既存ルール | 接続 |
|---|---|
| 1. 意思決定の優先順位 | 変更なし。プロトコルは優先順位3（＝明記のない詳細の判断手順）を具体化したもの |
| 2-2. 思弁的な機能を先回りして実装しない | プロトコル「4. 最小変更を優先」と同じ方向。ループを回すことは、スコープを広げてよい理由にならない |
| 2-5. 曖昧な時の対応 | プロトコル「6. 質問する前に調査する」で**上書きしない**。調べられることは調べ切った上で、影響の大きい未決事項は従来どおり止めて確認する |
| 2-6. UI実装はDESIGN.mdに従う | **自律ループはここで必ず止まる。**Implementation Rules（①〜⑤）の提出と承認は人間の承認ゲートであり、プロトコルの自律性はこれを迂回する根拠にならない |
| 2-7. テストと概念の扱い | `EVIDENCE`の残し方はGolden Tests（判例）の規約に従う。**歴史を書き換えない** |
| `rejected-decisions.md` | 却下した仮説と、**手順そのものの失敗**（例：証拠を取ると決めた直後に推測で修正した）を残す場所 |
| `CHANGELOG.md` | 事件→コミット→ファイルのTraceを残す場所 |

> **制定の経緯（2026-08-07）**：マーカーの選択境界バグの調査中、「ブラウザ側の仕様」「`key={i}`による
> DOM削除」を、証拠を取ると宣言した直後に**推測のまま原因として確定**する手順違反が2回起きた
> （記録：`rejected-decisions.md`）。いずれも外部レビューで反証された。このプロトコルは、
> その手順の失敗を構造的に防ぐために導入された。
...
- `EVIDENCE-OS.md`：**設計思想**（機能仕様ではない）。全ドキュメントの最上位。新機能・新AIを追加する際に「これは人間の思考を守るか、それとも奪うか」を判定する場所。7つの憲法・語彙の確定（Chronicle/Evidence）を含む。**この文書を根拠に現行コードをリファクタリングしないこと**（2026-07-28制定）
- `PRINCIPLES.md`：Lumoraの設計原則（5箇条）。個別の仕様判断で迷ったら最初に立ち返る場所（2026-07-23制定）
- `data-handling.md`：**データの取り扱いの設計契約**（2026-07-29制定）。保存先・外部送信の範囲・削除・保持期間。**この文書に書かれていないデータの流れを実装しないこと**。新しい外部送信・保存先・AIプロバイダを追加する時は実装より先にここを更新する。プライバシーポリシー等はここから生成する
- `service-map.md`：**サービス台帳**（2026-08-01制定）。採用済みインフラ（GitHub/Vercel/Supabase/Anthropic/Sentry等）の役割・依存関係・障害影響・離脱可能性を横断的に一覧化する。個別の採否判断は`infra.md`、横断的な地図は本書、と役割を分ける。新しい外部サービスを採用したら行を追加する
- `trust-model.md`：`data-handling.md`を利用者向けに翻訳した信頼境界の説明。食い違ったら`data-handling.md`が正
...
- `rejected-decisions.md`（リポジトリ直下）：却下した仮説と、その根拠。および**手順そのものの失敗**の記録。「一度否定した説をもう一度持ち出していないか」を確認する場所（自律開発プロトコル参照）
