...
**この原則が拘束しているのはシステムの側であって、ユーザーの側ではない（明確化・2026-07-29）。**
AI・Lumoraが勝手に知識を捨てないための条項であり、**ユーザー自身の削除権を制限するものではない**。
Principle 1（AIは提案者であり決定者ではない）と同じ方向の制約——主語はどちらもシステムである。

したがって、ユーザーが明示的に削除を求めた場合は物理削除してよい。保留一覧からの
「完全に削除」（2段階の明示操作、`data-model.md`）が既にその例であり、アカウント削除も同様に扱う。
「知識を預かるOS」にとって**いつでも離れられること自体が信頼の一部**であり、削除手段を持たないことは
この原則の帰結ではない。データの取り扱いの詳細は`docs/data-handling.md`。
