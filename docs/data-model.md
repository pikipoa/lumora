...
**語彙の確定（2026-07-28、`EVIDENCE-OS.md`付録B）**：設計思想文書`EVIDENCE-OS.md`の制定にあたり、上表の語と衝突しうる2語を確定した。

- **Chronicle**は上表の意味（Markerを含む会話＝文脈）で**固定**する。`EVIDENCE-OS.md` 9章が語る「思考の変化そのもの」は別概念であり、実装が無いため**現時点では命名しない**（実装される時点で改めて命名する）
- **Evidence**は思想レベルの語（Evidence OS / Evidence First / Evidence Map）に**限定**し、データモデル上の段階・テーブルにはしない。知識の最小単位は引き続き**Marker（＝Arca）**。「AIに渡す直前の集約」が必要になった場合はテーブルではなく**View**として設計する（下記「RealmとViewの責務分離」）

いずれも「実装が先にあって概念が後から名前をつける」という順序（`docs/future/mission-architecture.md` 5章）に従った判断である。
