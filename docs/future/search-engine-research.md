...
2. **ExplorerとSearchは兄弟であり、積み上げではない** — 両者に共通の述語（期間・source・採掘状態）は同一のFilter primitiveを呼ぶ。Realm/Wingは案A採用によりSearch側に述語自体が存在しないため対象外で、代わりにExplorerの件数とRealmページ/Wingページの一覧が単一の実装を共有する（D9）。どちらも「数える場所」と「見せる場所」が2箇所あるとき実装は1つでなければならない、という同じ原則の適用先が違うだけ（→5章）
