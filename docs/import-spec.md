...
対象：ChatGPT / Gemini / Claude / Perplexity / 汎用Markdown・テキスト文書（2026-07-14追加、5-1参照） / Claude Code CLIセッション記録（2026-07-21追加、5-2参照）
...
### 5-1. 汎用Markdown/テキスト文書（追加・2026-07-14）

`.md`/`.markdown`/`.txt`ファイルは形式判定の時点ではPerplexityパーサーへ渡す（拡張子だけでは区別できないため）。実際にPerplexityのQ/A構造（`## 見出し`）が見つかった場合のみ`source: "perplexity"`として扱い、見つからなければ**汎用のメモ・ドキュメントとして`source: "document"`で取り込む**（パーサー実装：`src/import/parsers/perplexity.ts`）。

**きっかけ**：AIチャット以外（例：外部で作成したロードマップ等のMarkdownファイル）もLumoraに取り込んで検索したい、という要望。`search-spec.md`「2章 一次情報」原則（ユーザー自身が作成またはインポートした一次情報は検索対象）に沿う形で、Phase1のスコープを「4社AIチャットのエクスポート」から「汎用のMarkdown/テキスト文書」まで広げた。

**パース仕様**：ファイル全体を1つの会話として取り込み、内容全体を1つのメッセージ（`role: "user"`）とする。先頭の`# 見出し`があればタイトルに、無ければファイル名をタイトルにする（Perplexityパーサーの既存ロジックをそのまま流用）。

**Phase1のスコープ**：Markdown/テキストのみ。画像OCR・PDF・音声・動画・Web記事等の抽出テキストはPhase2以降（`search-spec.md`「2-2」参照）。

### 5-2. Claude Code CLIセッション記録（追加・2026-07-21）

`.jsonl`ファイルは`source: "claude_code"`として取り込む（パーサー実装：`src/import/parsers/claudeCode.ts`）。

**きっかけ**：ユーザー自身が過去にClaude Code（本アプリの開発に使っているCLIエージェント）で行ったセッションも、一次情報としてLumoraに取り込みたいという要望。セッション記録は`C:\Users\<user>\.claude\projects\...\<sessionId>.jsonl`にローカル保存されている。

**フォーマットの特徴（実データ検証・2026-07-21）**：1行1JSONイベントのログ形式で、会話内容以外のセッション運営イベント（`system`/`mode`/`queue-operation`/`attachment`/`last-prompt`等）が大量に混在する非公式・非保証の内部フォーマット。

**パース仕様**：
- `type:"user"`は`message.content`が**文字列の場合のみ**実際のユーザー発言として採用する。配列（`tool_result`等）はツール実行結果の返送でありユーザーの発言ではないため除外
- `type:"assistant"`は`message.content`配列中の`type:"text"`ブロックのみを回答として採用する。`thinking`（内部思考）と`tool_use`（ツール呼び出しパラメータ）は取り込まない
- `type:"ai-title"`の`aiTitle`（セッション中に複数回更新される）を会話タイトルとし、最後に出現した値を採用する
- 上記のいずれにも該当しないイベント種別はすべて無視する（将来イベント種別が増えても壊れない）

**他のエージェントAIツールへの対応（要調査・未着手）**：同じJSONL構造を持つツールがあれば同じパーサーで対応できる可能性があるが、2026-07-21時点でこの開発環境を調査した結果は以下の通り：
- Codex CLI：ローカルにファイルは存在したが、中身はアプリ状態（オンボーディング状況等）のみで、セッション記録ではなかった
- Cursor / Cline（VS Code拡張）/ Aider / Gemini CLI / Windsurf：この環境に存在せず、実データを確認できていない

推測でパーサーを作ると「壊れても落ちない」原則に反するため、対応する場合は該当ツールの実データ（サンプルファイル）を確認してから着手する。
...
| 汎用Markdown/テキスト文書 | `.md`/`.txt`でPerplexity形式（Q/A見出し）が見つからない場合、`source: "document"`として1メッセージで取り込む（確定・2026-07-14、5-1参照） |
| Claude Code CLIセッション記録 | `.jsonl`を`source: "claude_code"`として取り込む（確定・2026-07-21、5-2参照）。他のエージェントAIツールは実データ未確認のため対応保留 |
