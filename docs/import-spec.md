...
### 実データ検証で確定した実際のスキーマ（2026-07-22、実際にエクスポートした1ターン/7ターンの2スレッドで確認）

**旧実装が想定していた「`## 見出し`＝ユーザーの質問、その下の本文＝AIの回答」という前提は誤りだった**：実データでは`##`はAIが自分の回答を整理するために使う小見出しであり、ユーザーの追加質問ではない。

実際に確認できた構造：
- 先頭に`<img src="https://r2cdn.perplexity.ai/...">`（Perplexityロゴ）が必ず付く。**実Perplexityエクスポートかどうかの判定はこのURLの有無（または脚注形式の引用マーカーの有無）で行う**（`## 見出し`の有無では判定できない。汎用文書も`##`を使いうるため）
- フォローアップ質問を含む複数ターンのスレッドは、**各ターンが`# 見出し`（H1）で始まり、ターン同士は`---`（水平線）のみの行で区切られる**（2回目以降の質問も`##`ではなく`#`）
- ユーザーの質問文（H1見出し行＋直後の補足行）とAIの回答本文の境界には明示的なマーカーが無いが、**引用マーカー`[^N_M]`（例：`[^1_1]`）を含む最初の段落からAIの回答が始まる**という一貫したパターンが検証した全7ターンで確認できたため、この境界検出に使う（マーカーが1つも無いターンは、見出し行のみを質問とし残り全体を回答として扱う）
- 引用URLの定義は`[^N_M]: https://...`（脚注形式）。ターンごとに番号がリセットされる（`^1_1`〜`^1_19`, `^2_1`…）ため、ターンのブロック単位で抽出すれば自然にスコープが分かれる
- 回答末尾に`<span style="display:none">[^N_x]...</span>`（未使用の脚注番号一覧）と`<div align="center">⁂</div>`（装飾区切り）が付く。どちらも表示上のノイズなので除去する
- 旧仕様書が想定していた`[1]: https://...`（連番のみ）形式の引用は今回のデータには存在しなかったが、後方互換のため抽出パターンとして残す

実装は`src/import/parsers/perplexity.ts`に反映済み。実データ2件（1ターン・7ターン、計8ターン）で検証、failed 0件。

### 5-1. 汎用Markdown/テキスト文書（追加・2026-07-14）

`.md`/`.markdown`/`.txt`ファイルは形式判定の時点ではPerplexityパーサーへ渡す（拡張子だけでは区別できないため）。実際にPerplexityのシグネチャ（ロゴ画像URLまたは脚注形式の引用マーカー）が見つかった場合のみ`source: "perplexity"`として扱い、見つからなければ**汎用のメモ・ドキュメントとして`source: "document"`で取り込む**（パーサー実装：`src/import/parsers/perplexity.ts`。判定方法は2026-07-22の実データ検証により`## 見出し`の有無から訂正済み、上記参照）。
