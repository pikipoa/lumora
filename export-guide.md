...
**この文書が原本です。** 同じ内容をアプリ内でも表示しており（S3.5 エクスポート手順画面）、
配信物の文言は `src/i18n/ja.ts` の `exportGuide` に置いてあります。
**手順を直すときは両方を更新してください。**

- 原本（開発・レビュー用）：`export-guide.md`（このファイル）
- 配信物（利用者が読むもの）：`src/i18n/ja.ts` の `exportGuide` → `src/app/export-guide.tsx`
