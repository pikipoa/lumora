...
- 「左ハンドルが最上部へ飛ぶ」原因を**ブラウザ側の仕様**とした結論（2026-08-05）、
  および続く**セグメントのkey={i}によるDOM削除**という機序（2026-08-06）。
  却下理由：どちらも証拠を取る前に確定させた推測だった。前者は実験結果
  （オートスクロールOFFなら壊れない）が示す「スクロールが引き金」までしか裏付けが
  無いのに、その先の機序を「ブラウザが画面外のハンドルを追えず選択を回復させる」と
  埋めた。後者は必要条件が2つとも成立しないことを後から確認した：
  (1) `computeSegments`の入力は`m.content`と`layersByMessage`（依存は`[markers, messages]`）
  のみで**選択中の状態は一切入らない**ため、選択中にセグメント構成は変わらず、
  `key={i}`でもテキストノードは削除されない。
  (2) `setScrolling(true)`は連続スクロール中ずっと同じ値なのでReactがbailoutし、
  「毎フレーム再レンダリング」も誤りだった。
  **現時点で確定しているのは「仮説②③ではない」ことだけ**（`caretRangeFromPoint`・
  `setBaseAndExtent`・`extend`・`addRange`の使用が0件であることをgrepで確認）。

  **手順の失敗として記録する**：「証拠を取る」と宣言した直後に、証拠を見ないまま
  修正をコミット・デプロイした。しかも2回連続で同じ失敗をした。テストが125件通過
  していることは、このバグについて何も語らない。**再現手順を固定し、修正前のコミットで
  100%再現することを先に示さない限り「直った」とは言えない。**
  なお`key={i}`から位置ベースのkeyへの変更自体は、マーカーが増減した時の無駄な
  再マウントを避ける改善として妥当なため残した（ただし原因の修正ではない）。
