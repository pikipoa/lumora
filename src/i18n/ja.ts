...
export const ja = {
  ...
  conversation: {
    ...
    roleUser: 'あなた',
    roleAssistant: 'AI',
    editingMarker: (excerpt: string) => `マーカーを編集：「${excerpt}」`,
    editingProposed: (excerpt: string) => `🤖 AI提案（Ore）を編集：「${excerpt}」`,
    newMarker: (excerpt: string) => `新規マーカーを作成：「${excerpt}」`,
    adjustHint: '範囲をドラッグして調整してから色を選ぶと、その範囲で確定します',
    reject: '却下',
